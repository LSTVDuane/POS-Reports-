...
adobo_cost = sheet_obj.cell(row = 11, column = 11)
adobo_profit = sheet_obj.cell(row = 11, column = 12)
sinigang_cost = sheet_obj.cell(row = 12, column = 11)
sinigang_profit = sheet_obj.cell(row = 12, column = 12)

totalCost = float(adobo_cost.value) + float(sinigang_cost.value)
totalProfit = float(adobo_profit.value) + float(sinigang_profit.value)
posTotalCost = sheet_obj.cell(row = 15, column = 11)
posTotalProfit = sheet_obj.cell(row = 15, column = 12)

pymsgbox.alert('                        Cost' + '           Profit' + '\n' + 'Adobo' + '          '+ str(adobo_cost.value) + '          ' + str(adobo_profit.value) + '\n' + 'Sinigang ' + '      ' + str(sinigang_cost.value) + '          ' + str(sinigang_profit.value) + '\n' + '\n' + 'Total Cost: ' + str(totalCost) + '\n' + 'POS Total Cost: ' + str(posTotalCost.value) + '\n' + 'Total Profit: ' + str(totalProfit) + '\n' + 'POS Total Profit: ' + str(posTotalProfit.value),timeout = 10000 )

if float(totalCost) == float(posTotalCost.value):
    pymsgbox.alert('Total Cost Match ✔️',timeout=3000)
else:
    pymsgbox.alert('Total Cost Not Match ❌',timeout=3000)   

if float(totalProfit) == float(posTotalProfit.value):
    pymsgbox.alert('Total Profit Match ✔️',timeout=3000)
else:
    pymsgbox.alert('Total Profit Not Match ❌',timeout=3000)  
